...
//加密函数(elgamal加密)，对内部加密函数进行了进一步的封装
void encrypt(ElGamal::Encryptor& encryptor,std::string& plaintext,std::string& ciphertext)
{
	...
	{
		if(i*str_size+str_size>plaintext.size()) plain[i]=plaintext.substr(i*str_size,plaintext.size()-i*str_size+1);
