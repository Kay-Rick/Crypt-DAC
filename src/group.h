...
void read_from_file(const string& filename,string& m)
{
	...
	if(!rfile) cout<<"open file fail in read_from_file"<<endl;
	int size;
...
void write_to_file(const string& filename,string& blocktext)
{
	...
	if(!wfile) cout<<"open file fail in write_to_file";
...
void send_to_server(){
    ...
    if(status==-1) cout<<"system()失败！"<<endl;
	/*
	int status=-1;
	int pid=fork();
	if(pid==0){
...
//元组序列化函数。将??地生成的元组对象序列化、签名、保存至本地文件、上传文件。
template<class T>
...
void serial_to_file(RK& tuple)
{
	...
	if(!os_file) cout<<"open file error"<<endl;
...
void unserial_from_file(T& tuple,const std::string& filename)
{
	...
	if(!is_file) cout<<"open file fail in unserial_from_file";
...
public:
		...
		void serial()
        {
			...
			if(!os_file) cout<<"open USER file fail";
			boost::archive::binary_oarchive oa(os_file);
			oa << *this;
			os_file.close();
        }
		...
		void unserial(std::string filename)//f
        {
			...
			if(!is_file) cout<<"open USER file fail";
			boost::archive::binary_iarchive ia(is_file);
...
public:
        ...
        if(!os_file) cout<<"open file fail in role serial";
        boost::archive::binary_oarchive oa(os_file);
        oa << *this;
        os_file.close();
        }
        ...
        if(!is_file) cout<<"open file fail in role unserial";
        boost::archive::binary_iarchive ia(is_file);
        ia >> *this;//序列化到一个ostd::stringstream里面
        is_file.close();
        }
...
public:
        ...
        void serial()
        {
			...
			if(!os_file) cout<<"open file fail in file serial";
			boost::archive::binary_oarchive oa(os_file);
			oa << *this;
			os_file.close();
        }
        void unserial(std::string filename)//f
        {
			...
			if(!is_file) cout<<"open file fail in file unserial";
			boost::archive::binary_iarchive ia(is_file);
